...
from coqui_tts.utils.text.symbols import symbols
from coqui_tts.inference import AutoProcessor
...
def text_to_speech(text, output_path, verbose=False):
    processor = AutoProcessor.from_pretrained("tts_models/en/ljspeech/tacotron2-DDC/config.json")
    text = text[:250]  # Limit text to 250 characters due to CoquiTTS limitations
    wav = processor.tts(text)
    sf.write(output_path, wav, processor.config.sampling_rate)
    if verbose:
...
def convert_file_to_audiobook(file_path, output_path, verbose=False):
    ...
    for i, chapter_text in enumerate(tqdm(chapters, desc="Converting chapters")):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Convert PDF or EPUB to Audiobook using CoquiTTS')
